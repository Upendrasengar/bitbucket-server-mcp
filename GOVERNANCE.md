...
The current maintainer is **Upendra Sengar**
([@upendrasengar](https://github.com/upendrasengar)).
...
- **Repository ownership**: The repo is under
  [upendrasengar](https://github.com/upendrasengar). If needed, GitHub
  Support can transfer it to a designated successor.
- **Publishing rights**: The package `bitbucket-server-mcp` is on npm under
  `@upendrasengar`. Releases are published from the `release.yml` workflow via
