function parseDiff(lines) {
    const sections = [];
    let current = { fileName: "", headerLines: [], hunkContent: [] };
    let inHunk = false;
...
function truncateHunkContent(hunkContent, fileName, maxLines) {
...
export function truncateDiff(diffContent, maxLinesPerFile) {
    ...
    return sections
        .flatMap((section) => [
        ...section.headerLines,
        ...truncateHunkContent(section.hunkContent, section.fileName, maxLinesPerFile),
    ])
        .join("\n");
}
