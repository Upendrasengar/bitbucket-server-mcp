                    ...
                    if (!response.ok) {
                        if (response.status === 429) {
                            ...
                            if (reset) {
                                ...
                                if (waitMs > 0) {
                                    logger.warn(`Rate limited (429); reset in ${waitMs}ms, retry handled by HTTP layer`);
                                }
                            }
                        }
...
export function getPaginated(client, url, options) {
    return client
        .get(url, options)
        .json()
