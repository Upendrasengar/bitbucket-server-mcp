...
export const DEFAULT_PR_FIELDS = "id,title,description,state,createdDate,updatedDate," +
...
export const DEFAULT_REPOSITORY_FIELDS = "slug,id,name,description,state,forkable,project.key,project.name";
export const DEFAULT_BRANCH_FIELDS = "id,displayId,type,latestCommit,isDefault,metadata";
export const DEFAULT_SEARCH_FIELDS = "file,hitCount,hitContexts,pathMatches," +
    "repository.slug,repository.name,repository.project.key";
export const DEFAULT_COMMIT_FIELDS = "id,displayId,message," +
...
export const DEFAULT_ACTIVITY_FIELDS = "id,action,createdDate," +
    "user.name,user.displayName," +
    "comment.id,comment.text,comment.createdAt,comment.updatedAt";
export const DEFAULT_USER_FIELDS = "name,displayName,emailAddress,slug,active,type";
...
export const DEFAULT_DEPLOYMENT_FIELDS = "key,state,displayName,description,url," +
    "deploymentSequenceNumber,lastUpdated";
export const DEFAULT_COMMENT_FIELDS = "id,text,version,createdDate,updatedDate," +
    "author.name,author.displayName,severity,state," +
    "anchor.path,anchor.line,anchor.lineType";
export const DEFAULT_INSIGHT_FIELDS = "key,result,createdDate,details,link,title,reporter";
export const DEFAULT_REVIEWER_FIELDS = "id,scope.type,reviewers.name," +
    "sourceMatcher.type,sourceMatcher.displayId," +
    "targetMatcher.type,targetMatcher.displayId";
function pickFieldsFromObject(source, fieldPaths) {
        ...
        if (subPaths.length === 0) {
            ...
            result[key] = value.map((item) => {
                if (item && typeof item === "object") {
                    return pickFieldsFromObject(item, subPaths);
                }
                return item;
            });
        }
            ...
            result[key] = pickFieldsFromObject(value, subPaths);
        }
    }
    return result;
}
 ...
 */
export function curateResponse(data, fields) {
    if (fields === "*all") {
        return data;
    }
 ...
 */
export function curateList(items, fields) {
    if (fields === "*all") {
        return items;
    }
