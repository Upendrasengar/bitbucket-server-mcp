...
const arbitraryObject = fc.dictionary(fc.stringMatching(/^[a-z][a-zA-Z0-9]*$/), fc.oneof(fc.string(), fc.integer(), fc.boolean(), fc.constant(null)));
describe("curateResponse (property-based)", () => {
    test.prop([arbitraryObject])("*all should return the original object unchanged", (data) => {
        expect(curateResponse(data, "*all")).toEqual(data);
    });
    test.prop([arbitraryObject])("empty fields should return empty object", (data) => {
        expect(curateResponse(data, "")).toEqual({});
    });
    test.prop([
        fc.dictionary(fc.stringMatching(/^[a-z][a-zA-Z0-9]*$/), fc.oneof(fc.string(), fc.integer(), fc.boolean(), fc.constant(null)), { minKeys: 1 }),
    ...
    test.prop([arbitraryObject])("requesting non-existent fields should produce empty object", (data) => {
        const result = curateResponse(data, "nonExistentField123");
        expect(result).toEqual({});
    });
    test.prop([fc.array(arbitraryObject, { minLength: 0, maxLength: 10 })])("curateList with *all should return the original array", (items) => {
    ...
    ])("nested field picking should extract sub-paths and discard others", (data) => {
        const result = curateResponse(data, "id,author.name,author.email");
    ...
    test.prop([
    ...
    ])("array elements should have sub-fields picked individually", (reviewers) => {
        ...
        expect(result.reviewers).toEqual(reviewers.map(({ name, status }) => ({ name, status })));
    });
});
//# sourceMappingURL=curate.prop.test.js.map