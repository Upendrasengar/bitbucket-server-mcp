...
const errorField = fc.oneof(fc.constant(undefined), fc.string({ minLength: 1, maxLength: 80 }));
const restErrorMessage = fc.record({
    message: errorField,
    context: errorField,
    exceptionName: errorField,
}, { requiredKeys: [] });
const restErrors = fc.record({
...
const restErrorsWithMessage = fc
    .tuple(fc.record({
    message: fc.string({ minLength: 1, maxLength: 80 }),
    context: errorField,
    exceptionName: errorField,
}, { requiredKeys: ["message"] }), fc.array(restErrorMessage, { minLength: 0, maxLength: 3 }))
...
describe("extractBitbucketMessage (property-based over spec-typed bodies)", () => {
    ...
    test.prop([restErrorsWithMessage])("bodies with at least one message produce a non-empty result", (body) => {
        expect(extractBitbucketMessage(body).length).toBeGreaterThan(0);
    });
    test.prop([fc.string({ minLength: 501, maxLength: 20_000 })])("string body longer than the cap is truncated, not dropped", (body) => {
...
//# sourceMappingURL=errors.prop.test.js.map