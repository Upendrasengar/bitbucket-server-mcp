...
describe("Tool schema contract: every field has a non-empty description", () => {
    ...
    test("every tool's input fields have non-empty descriptions", async () => {
        const { tools } = await client.listTools();
        const withSchema = tools.filter((t) => !toolsWithoutInputSchema.has(t.name));
        for (const tool of withSchema) {
            const props = tool.inputSchema.properties ?? {};
            const fields = Object.entries(props);
            expect(fields.length, `${tool.name} has no input fields`).toBeGreaterThan(0);
            for (const [fieldName, schema] of fields) {
                expect(schema.description, `${tool.name}.${fieldName} has no description`).toBeDefined();
                expect(schema.description.length, `${tool.name}.${fieldName} has empty description`).toBeGreaterThan(0);
            }
        }
    });
});
...
//# sourceMappingURL=tool-schema-contract.test.js.map