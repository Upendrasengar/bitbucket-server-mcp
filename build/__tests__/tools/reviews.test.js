...
import { callAndParse, expectCalledWithJson, setupToolHarness, } from "../tool-test-utils.js";
describe("Review tools", () => {
    ...
    describe("manage_review", () => {
        test("should approve a pull request", async () => {
            ...
            const parsed = await callAndParse(h.client, "manage_review", {
                action: "approve",
                repository: "my-repo",
                prId: "42",
            });
            expect(parsed.approved).toBe(true);
            expect(h.mockClients.api.post).toHaveBeenCalledWith("projects/DEFAULT/repos/my-repo/pull-requests/42/approve", { json: {} });
        });
        test("should unapprove a pull request", async () => {
            ...
            expect(h.mockClients.api.delete).toHaveBeenCalledWith("projects/DEFAULT/repos/my-repo/pull-requests/42/approve");
        });
        test("should publish a review with participantStatus APPROVED", async () => {
            ...
            expectCalledWithJson(h.mockClients.api.put, "projects/DEFAULT/repos/my-repo/pull-requests/42/review", {
                commentText: "LGTM",
                participantStatus: "APPROVED",
            });
        });
    ...
    describe("manage_review publish (decision table: commentText x participantStatus)", () => {
        test.each([
            ...
            expect(h.mockClients.api.put).toHaveBeenCalledWith("projects/DEFAULT/repos/r/pull-requests/1/review", { json: expectedBody });
        });
    });
    describe("manage_review action-to-verb mapping over sequences", () => {
        test("approve then unapprove issues POST then DELETE in that order", async () => {
            ...
            expect(h.mockClients.api.post).toHaveBeenCalledWith("projects/DEFAULT/repos/r/pull-requests/1/approve", { json: {} });
            expect(h.mockClients.api.delete).toHaveBeenCalledWith("projects/DEFAULT/repos/r/pull-requests/1/approve");
        });
    ...
    describe("manage_review URL construction (grey box)", () => {
        test.each([
            ...
            expect(h.mockClients.api.post).toHaveBeenCalledWith(`projects/${args.project}/repos/${args.repository}/pull-requests/${args.prId}/approve`, { json: {} });
        });
