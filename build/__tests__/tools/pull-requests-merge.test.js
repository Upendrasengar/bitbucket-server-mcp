...
describe("Pull request tools", () => {
    ...
    describe("merge_pull_request", () => {
        test("should fetch version and merge", async () => {
            ...
            const parsed = await callAndParse(h.client, "merge_pull_request", {
                project: "PROJ",
                repository: "my-repo",
                prId: 5,
            ...
            expect(h.mockClients.api.get).toHaveBeenCalledWith("projects/PROJ/repos/my-repo/pull-requests/5");
            expect(h.mockClients.api.post).toHaveBeenCalledWith("projects/PROJ/repos/my-repo/pull-requests/5/merge", expect.objectContaining({
        ...
        test("version GET fails", async () => {
            ...
            const r = await callRaw(h.client, "merge_pull_request", {
                project: "PROJ",
                repository: "my-repo",
                prId: 5,
            });
    ...
    describe("merge_pull_request (equivalence classes over strategy enum)", () => {
        test.each([
            ...
            expectCalledWithSearchParams(h.mockClients.api.post, "projects/PROJ/repos/my-repo/pull-requests/10/merge", { strategyId: strategy });
        });
        test("omits strategyId when strategy not provided", async () => {
            ...
            const [, opts] = h.mockClients.api.post.mock.calls[0];
            expect(opts?.searchParams?.strategyId).toBeUndefined();
        });
    });
});
//# sourceMappingURL=pull-requests-merge.test.js.map