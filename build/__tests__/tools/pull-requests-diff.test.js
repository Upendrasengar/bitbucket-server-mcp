...
describe("Pull request tools", () => {
    ...
    describe("get_diff", () => {
        ...
        test("should pass contextLines and withComments to searchParams", async () => {
            ...
            expect(h.mockClients.api.get).toHaveBeenCalledWith("projects/PROJ/repos/my-repo/pull-requests/1/diff", expect.objectContaining({
                searchParams: { contextLines: 5, withComments: false },
                headers: { Accept: "text/plain" },
            }));
        });
        test("should not truncate when maxLinesPerFile is 0", async () => {
            const rawDiff = "diff --git a/big.ts b/big.ts\n" +
                "index abc..def 100644\n" +
                "--- a/big.ts\n" +
                "+++ b/big.ts\n" +
                "@@ -1 +1 @@\n" +
        ...
        test("should return file list with stat=true", async () => {
            ...
            const parsed = await callAndParse(h.client, "get_diff", {
                project: "PROJ",
                repository: "my-repo",
                prId: 1,
                stat: true,
            ...
            expectCalledWithSearchParams(h.mockClients.api.get, expect.stringContaining("/changes"), { limit: 1000 });
        });
        test("should include summary when diff-stats-summary is available", async () => {
            ...
            expect(h.mockClients.api.get).toHaveBeenCalledWith("projects/PROJ/repos/my-repo/pull-requests/1/diff-stats-summary");
        });
        test("appends filePath to URL when provided", async () => {
            ...
            expect(h.mockClients.api.get).toHaveBeenCalledWith("projects/PROJ/repos/my-repo/pull-requests/1/diff/src/index.ts", expect.anything());
        });
...
//# sourceMappingURL=pull-requests-diff.test.js.map