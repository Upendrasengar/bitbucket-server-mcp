...
describe("manage_deployments", () => {
    ...
    describe("get", () => {
        test("returns deployment from API with correct searchParams", async () => {
            ...
            const parsed = await callAndParse(h.client, "manage_deployments", {
                action: "get",
                ...GET_REQUIRED_PARAMS,
            });
        ...
        ])("returns error when %s is missing", async (_, args) => {
    ...
    describe("create", () => {
        ...
        test("create without environmentType omits it from body", async () => {
            ...
            const body = h.mockClients.api.post.mock.calls[0][1];
            expect(body.json.environment.type).toBeUndefined();
        });
    ...
    test("uses default project when not provided", async () => {
        ...
        expect(h.mockClients.api.get).toHaveBeenCalledWith("projects/DEFAULT/repos/my-repo/commits/abc123/deployments", expect.anything());
    });
    test("uses explicit project when provided", async () => {
        ...
        expect(h.mockClients.api.get).toHaveBeenCalledWith(BASE_URL, expect.anything());
    });
