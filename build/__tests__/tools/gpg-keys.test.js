...
describe("list_gpg_keys", () => {
    ...
    test("returns full response shape", async () => {
        ...
        const p = await callAndParse(h.client, "list_gpg_keys", {});
...
describe("manage_gpg_keys", () => {
    ...
    test("add key", async () => {
        ...
        const p = await callAndParse(h.client, "manage_gpg_keys", {
            action: "add",
            text: "-----BEGIN...",
        });
        expect(p.id).toBe(1);
    ...
    test("delete key constructs URL and returns keyId", async () => {
        ...
        const p = await callAndParse(h.client, "manage_gpg_keys", { action: "delete", keyId: 42 });
...
//# sourceMappingURL=gpg-keys.test.js.map