...
describe("list_webhooks", () => {
    ...
    test("returns webhooks from the API", async () => {
        ...
        const parsed = await callAndParse(h.client, "list_webhooks", {
            project: "TEST",
            repository: "my-repo",
        });
        expect(parsed.total).toBe(1);
        expect(parsed.webhooks[0].name).toBe("ci-hook");
        expect(h.mockClients.api.get).toHaveBeenCalledWith("projects/TEST/repos/my-repo/webhooks", expect.objectContaining({ searchParams: { limit: 25, start: 0 } }));
    });
...
describe("manage_webhooks", () => {
    ...
    test("creates a webhook", async () => {
        ...
        const parsed = await callAndParse(h.client, "manage_webhooks", {
            action: "create",
            project: "TEST",
            repository: "my-repo",
            name: "my-hook",
        ...
        expectCalledWithJson(h.mockClients.api.post, "projects/TEST/repos/my-repo/webhooks", {
            name: "my-hook",
            url: "https://example.com/hook",
            events: ["repo:refs_changed"],
        });
    ...
    test("creates a webhook without active omits it from body", async () => {
        ...
        expect(h.mockClients.api.post).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({
    ...
    test.each([
    ...
    ])("updates a webhook with $description", async ({ extraArgs, expectedJson }) => {
        ...
        expectCalledWithJson(h.mockClients.api.put, "projects/TEST/repos/my-repo/webhooks/1", expectedJson);
    });
