...
describe("Pull request tools", () => {
    ...
    describe("get_pull_request_activity", () => {
        test("should return all activities with pagination info", async () => {
            ...
            const parsed = await callAndParse(h.client, "get_pull_request_activity", {
                project: "PROJ",
                repository: "my-repo",
                prId: 1,
            });
        ...
        test("should forward limit and start as searchParams", async () => {
            ...
            expectCalledWithSearchParams(h.mockClients.api.get, expect.stringContaining("/activities"), { limit: 50, start: 10 });
        });
        test("should filter to reviews only", async () => {
            ...
            expect(parsed.activities.every((a) => a.action === "APPROVED" || a.action === "REVIEWED")).toBe(true);
        });
        test("should filter to comments only", async () => {
            ...
            expect(parsed.activities).toHaveLength(2);
            expect(parsed.activities.every((a) => a.action === "COMMENTED")).toBe(true);
        });
...
//# sourceMappingURL=pull-requests-activity.test.js.map