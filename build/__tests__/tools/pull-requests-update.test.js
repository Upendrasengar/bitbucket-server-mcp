...
import { callAndParse, callRaw, expectCalledWithJson, expectCalledWithStrictJson, setupToolHarness, } from "../tool-test-utils.js";
describe("Pull request tools", () => {
    ...
    describe("update_pull_request", () => {
        test("should preserve reviewers when not provided", async () => {
            ...
            const parsed = await callAndParse(h.client, "update_pull_request", {
                project: "PROJ",
                repository: "my-repo",
                prId: 10,
                title: "New title",
            ...
            expectCalledWithJson(h.mockClients.api.put, "projects/PROJ/repos/my-repo/pull-requests/10", {
                title: "New title",
                reviewers: [{ user: { name: "bob" }, status: "APPROVED" }],
            });
        });
        ...
        test("should update target branch preserving repository info for cross-repo PRs", async () => {
            ...
            expect(h.mockClients.api.get).toHaveBeenCalledWith("projects/PROJ/repos/my-repo/pull-requests/10");
            expectCalledWithJson(h.mockClients.api.put, expect.any(String), {
                toRef: {
        ...
        ])("preserves title/description when only reviewers updated (title=$title)", async ({ title, description, reviewer }) => {
            const existingPr = {
                id: 10,
                version: 5,
                title,
        ...
        test("should not send author field in PUT body", async () => {
            ...
            expectCalledWithStrictJson(h.mockClients.api.put, "projects/PROJ/repos/my-repo/pull-requests/12", {
                id: 12,
                version: 2,
                title: "New title",
                description: "Old desc",
...
//# sourceMappingURL=pull-requests-update.test.js.map