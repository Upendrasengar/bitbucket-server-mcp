...
import { callAndParse, callRaw, expectCalledWith, expectCalledWithSearchParams, setupToolHarness, } from "../tool-test-utils.js";
describe("get_user_profile", () => {
    ...
    test("returns user profile", async () => {
        ...
        const parsed = await callAndParse(h.client, "get_user_profile", { userSlug: "jdoe" });
    ...
    test("fields=*all returns the raw response including avatarUrl", async () => {
        ...
        const parsed = await callAndParse(h.client, "get_user_profile", { userSlug: "jdoe", fields: "*all" });
...
describe("search_users", () => {
    ...
    test("returns users matching filter", async () => {
        ...
        const parsed = await callAndParse(h.client, "search_users", { filter: "admin" });
