...
import { callAndParse, callRaw, expectCalledWithSearchParams, setupToolHarness, } from "../tool-test-utils.js";
describe("Branch tools", () => {
    ...
    describe("list_branches", () => {
        test("should list branches and include default branch", async () => {
            ...
            const parsed = await callAndParse(h.client, "list_branches", {
                project: "TEST",
                repository: "my-repo",
            });
        ...
        test("should use default project when not provided", async () => {
            ...
            expect(h.mockClients.api.get).toHaveBeenCalledWith("projects/DEFAULT/repos/my-repo/branches", expect.anything());
        });
        ...
        test("should handle default branch fetch failure gracefully", async () => {
            ...
            const parsed = await callAndParse(h.client, "list_branches", { project: "TEST", repository: "my-repo" });
        ...
        test("should fetch default branch from the correct endpoint", async () => {
            ...
            expect(h.mockClients.api.get).toHaveBeenCalledWith("projects/PROJ/repos/repo/default-branch");
        });
    });
    describe("list_commits", () => {
        test("should list commits for a branch", async () => {
            ...
            const parsed = await callAndParse(h.client, "list_commits", {
                project: "TEST",
                repository: "my-repo",
                branch: "main",
            });
            ...
            expectCalledWithSearchParams(h.mockClients.api.get, "projects/TEST/repos/my-repo/commits", { until: "main", limit: 25, start: 0 });
        });
        ...
        test("should use default project when not provided", async () => {
            ...
            expect(h.mockClients.api.get).toHaveBeenCalledWith("projects/DEFAULT/repos/my-repo/commits", expect.anything());
        });
    });
    describe("list_branches (filterText + pagination params forwarding)", () => {
        const searchParamsOfBranchesCall = () => {
            const callArgs = h.mockClients.api.get.mock.calls.find((c) => String(c[0]).endsWith("/branches"));
            expect(callArgs, "expected a /branches request to have been made").toBeDefined();
            return callArgs[1]
                .searchParams;
        };
        test.each([
        ...
        ])("passes filterText=$filterText, limit=$limit, start=$start", async ({ filterText, limit, start }) => {
    ...
    describe("list_commits author filter (equivalence: case + partial match)", () => {
        const commitWithAuthor = (name, displayName) => ({
            id: `sha${name}`,
            displayId: name,
            message: "c",
            authorTimestamp: 1,
        ...
        test("matches on displayName too", async () => {
            ...
            const parsed = await callAndParse(h.client, "list_commits", { repository: "r", author: "smith" });
            expect(parsed.commits).toHaveLength(1);
        });
        test("matches on slug field", async () => {
            ...
            const parsed = await callAndParse(h.client, "list_commits", { repository: "r", author: "dev-user" });
            expect(parsed.commits).toHaveLength(1);
        });
        test("excludes commits without author", async () => {
            ...
            const parsed = await callAndParse(h.client, "list_commits", { repository: "r", author: "alice" });
            expect(parsed.commits).toHaveLength(1);
        });
    });
    describe("list_branch_restrictions", () => {
        test("returns branch restrictions", async () => {
            ...
            const parsed = await callAndParse(h.client, "list_branch_restrictions", {
                project: "TEST",
                repository: "my-repo",
            });
            expect(parsed.total).toBe(1);
