...
import { callAndParse, callRaw, expectCalledWith, setupToolHarness, } from "../tool-test-utils.js";
describe("manage_branches", () => {
    ...
    describe("create", () => {
        test("creates a branch with startPoint", async () => {
            ...
            const parsed = await callAndParse(h.client, "manage_branches", {
                action: "create",
                project: "TEST",
                repository: "my-repo",
                branch: "feature/new",
            ...
            expect(h.mockClients.branchUtils.post).toHaveBeenCalledWith("projects/TEST/repos/my-repo/branches", {
                json: {
        ...
        test("uses default project when not provided", async () => {
            ...
            expect(h.mockClients.branchUtils.post).toHaveBeenCalledWith("projects/DEFAULT/repos/my-repo/branches", expect.anything());
        });
        ...
        test("returns error when branchUtils.post rejects", async () => {
            mockReject(h.mockClients.branchUtils.post, new Error("Invalid branch name"));
    ...
    describe("delete", () => {
        test("deletes a non-default branch", async () => {
            ...
            expect(h.mockClients.branchUtils.post).toHaveBeenCalledWith("projects/TEST/repos/my-repo/branches", { json: { name: "refs/heads/feature/old", dryRun: false } });
        });
        ...
        test("returns error when branchUtils.post fails after default-branch check", async () => {
            ...
            mockReject(h.mockClients.branchUtils.post, new Error("Internal server error"));
            const result = await callRaw(h.client, "manage_branches", {
                action: "delete",
                project: "TEST",
                repository: "my-repo",
        ...
        test("uses default project when not provided", async () => {
            ...
            expectCalledWith(h.mockClients.api.get, "projects/DEFAULT/repos/my-repo/default-branch");
        });
...
//# sourceMappingURL=branches-manage.test.js.map