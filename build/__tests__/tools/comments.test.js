...
import { callAndParse, callRaw, expectCalledWithJson, expectCalledWithSearchParams, setupToolHarness, } from "../tool-test-utils.js";
describe("Comment tools", () => {
    ...
    describe("manage_comment", () => {
        test("should create a general comment", async () => {
            ...
            const parsed = await callAndParse(h.client, "manage_comment", {
                action: "create",
                repository: "my-repo",
                prId: "42",
                text: "Looks good!",
        ...
        test("create without optional fields omits them from serialized body", async () => {
            ...
            const json = h.mockClients.api.post.mock.calls[0][1].json;
        ...
        test("edit without state/severity omits them from body", async () => {
            ...
            const body = h.mockClients.api.put.mock.calls[0][1].json;
    ...
    describe("manage_comment state field propagation", () => {
        test("PENDING -> OPEN via edit with state update", async () => {
            ...
            expectCalledWithJson(h.mockClients.api.put, "projects/DEFAULT/repos/r/pull-requests/1/comments/1", { state: "OPEN" });
        });
        test("OPEN -> RESOLVED -> OPEN cycle uses PUT with correct state", async () => {
            ...
            const firstCall = h.mockClients.api.put.mock.calls[0][1];
            const secondCall = h.mockClients.api.put.mock.calls[1][1];
        ...
        test("create-as-PENDING then edit to OPEN sends PENDING on POST and OPEN on PUT", async () => {
            ...
            const postBody = h.mockClients.api.post.mock.calls[0][1].json;
            const putBody = h.mockClients.api.put.mock.calls[0][1].json;
    ...
    describe("manage_comment threadResolved propagation (decision table)", () => {
        ...
        test.each([
        ...
        ])("threadResolved=$label produces the expected PUT body", async ({ extra, expectedBody }) => {
            ...
            const actualBody = h.mockClients.api.put.mock.calls[0][1].json;
    ...
    describe("manage_comment react/unreact", () => {
        test("react returns { react: true, commentId, emoticon }", async () => {
            ...
            expect(h.mockClients.commentLikes.put).toHaveBeenCalledWith(expect.stringContaining("/comments/5/reactions/thumbsup"));
        ...
        test("unreact returns { unreact: true, commentId, emoticon }", async () => {
            ...
            expect(h.mockClients.commentLikes.delete).toHaveBeenCalledWith(expect.stringContaining("/comments/5/reactions/thumbsup"));
            const parsed = JSON.parse(result.content[0].text);
            expect(parsed).toEqual({
                unreact: true,
                commentId: 5,
    ...
    describe("search_emoticons", () => {
        test("returns list of matching emoticon shortcuts", async () => {
            ...
            const parsed = await callAndParse(h.client, "search_emoticons", {
                query: "thumbs",
            });
