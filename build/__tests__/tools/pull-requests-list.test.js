...
import { callAndParse, expectCalledWithSearchParams, setupToolHarness, } from "../tool-test-utils.js";
describe("Pull request tools", () => {
    ...
    describe("list_pull_requests", () => {
        test("should list pull requests with filters", async () => {
            ...
            const parsed = await callAndParse(h.client, "list_pull_requests", {
                project: "PROJ",
                repository: "my-repo",
                state: "OPEN",
            });
        ...
        test("forwards direction and order as searchParams", async () => {
            ...
            expectCalledWithSearchParams(h.mockClients.api.get, expect.stringContaining("/pull-requests"), { direction: "OUTGOING", order: "NEWEST" });
        });
    });
    describe("list_pull_requests (each-value coverage of state/direction/order)", () => {
        test.each([
            ...
            expectCalledWithSearchParams(h.mockClients.api.get, expect.stringContaining("/pull-requests"), { state: args.state, direction: args.direction, order: args.order });
        });
    });
    describe("pagination params forwarding (limit/start)", () => {
        test.each([
            ...
            expectCalledWithSearchParams(h.mockClients.api.get, expect.any(String), args);
        });
    });
});
//# sourceMappingURL=pull-requests-list.test.js.map