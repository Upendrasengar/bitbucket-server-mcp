...
describe("mergeDefaultReviewers (decision table)", () => {
    describe("same source and target repo", () => {
        test("skips target repo lookup when srcProject=resolvedProject and srcRepo=repository", async () => {
            ...
            const result = await mergeDefaultReviewers(defaultParams({
                clients,
                resolvedProject: "P",
                repository: "r",
                srcProject: "P",
        ...
        test("fetches source repo by correct URL", async () => {
            ...
            await mergeDefaultReviewers(defaultParams({
                clients,
                srcProject: "SRC",
                srcRepo: "src-repo",
                resolvedProject: "SRC",
            ...
            expect(clients.api.get).toHaveBeenCalledWith("projects/SRC/repos/src-repo");
        });
        test("passes correct searchParams to defaultReviewers endpoint", async () => {
            ...
            expect(clients.defaultReviewers.get).toHaveBeenCalledWith("projects/P/repos/r/reviewers", expect.objectContaining({
    ...
    describe("cross-repo (source != target)", () => {
        test("fetches both source and target repo IDs", async () => {
            ...
            expect(clients.api.get).toHaveBeenCalledWith("projects/SRC/repos/src-repo");
            expect(clients.api.get).toHaveBeenCalledWith("projects/TGT/repos/tgt-repo");
        });
        test("uses different source and target repo IDs in searchParams", async () => {
            ...
            expect(clients.defaultReviewers.get).toHaveBeenCalledWith("projects/TGT/repos/tgt-repo/reviewers", expect.objectContaining({
                searchParams: expect.objectContaining({
                    sourceRepoId: 10,
                    targetRepoId: 20,
