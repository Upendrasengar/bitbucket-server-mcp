...
describe("Insight tools", () => {
    ...
    describe("get_code_insights", () => {
        test("should fetch reports and annotations", async () => {
            ...
            const parsed = await callAndParse(h.client, "get_code_insights", {
                project: "TEST",
                repository: "my-repo",
                prId: 1,
            });
        ...
        test("should use default project when not provided", async () => {
            ...
            expect(h.mockClients.insights.get).toHaveBeenCalledWith("projects/DEFAULT/repos/my-repo/pull-requests/1/reports");
        });
        test("should default annotations to empty array on error", async () => {
            ...
            mockJson(h.mockClients.insights.get, mockReports);
            mockError(h.mockClients.insights.get, new Error("Annotations not available"));
            const parsed = await callAndParse(h.client, "get_code_insights", {
                project: "TEST",
                repository: "my-repo",
                prId: 1,
        ...
        test("should query annotations for each report by key", async () => {
            ...
            const urls = h.mockClients.insights.get.mock.calls.map((c) => String(c[0]));
        ...
        test("should return file annotations keyed by path when includeFileAnnotations is true", async () => {
            ...
            const fileAnnotationCalls = h.mockClients.insights.get.mock.calls.filter((c) => String(c[0]).endsWith("/annotations") &&
                !String(c[0]).includes("/reports/"));
            expect(fileAnnotationCalls).toHaveLength(2);
            expect(fileAnnotationCalls[0][0]).toBe("projects/TEST/repos/my-repo/pull-requests/1/annotations");
            expect(fileAnnotationCalls[0][1]).toEqual(expect.objectContaining({
                searchParams: { path: "src/foo.ts", annotationLocation: "FILES" },
            }));
            expect(fileAnnotationCalls[1][0]).toBe("projects/TEST/repos/my-repo/pull-requests/1/annotations");
            expect(fileAnnotationCalls[1][1]).toEqual(expect.objectContaining({
        ...
        test("should return empty array for a file whose annotations fetch fails", async () => {
            ...
            mockError(h.mockClients.insights.get, new Error("Annotations unavailable"));
            // when
            const parsed = await callAndParse(h.client, "get_code_insights", {
                project: "TEST",
                repository: "my-repo",
    ...
    describe("get_build_status", () => {
        test("should fetch build status by commit ID", async () => {
            ...
            const parsed = await callAndParse(h.client, "get_build_status", { commitId: "abc123def456" });
            expect(parsed).toHaveLength(1);
            expect(parsed[0].state).toBe("SUCCESSFUL");
            expect(parsed[0].url).toBe("https://jenkins.example.com/job/123");
            expect(h.mockClients.buildStatus.get).toHaveBeenCalledWith("commits/abc123def456");
        });
        test("should resolve latest commit from PR and fetch build status", async () => {
            ...
            const parsed = await callAndParse(h.client, "get_build_status", { project: "PROJ", repository: "my-repo", prId: 42 });
            expect(parsed[0].state).toBe("FAILED");
            expect(h.mockClients.api.get).toHaveBeenCalledWith("projects/PROJ/repos/my-repo/pull-requests/42");
            expect(h.mockClients.buildStatus.get).toHaveBeenCalledWith("commits/resolved999");
        });
        ...
        test("should return empty array when commit has no build statuses", async () => {
            ...
            const parsed = await callAndParse(h.client, "get_build_status", { commitId: "abc123" });
            expect(parsed).toEqual([]);
        });
        test.each(["SUCCESSFUL", "FAILED", "INPROGRESS"])("returns build status %s correctly", async (state) => {
            mockJson(h.mockClients.buildStatus.get, {
                values: [{ state, name: "build", url: "https://ci.example.com" }],
            });
            const parsed = await callAndParse(h.client, "get_build_status", { commitId: "abc" });
        ...
        test("prId takes precedence over commitId when both provided", async () => {
            ...
            await callAndParse(h.client, "get_build_status", {
                project: "P",
                repository: "R",
                prId: 1,
                commitId: "should-be-ignored",
            });
            expect(h.mockClients.api.get).toHaveBeenCalledWith("projects/P/repos/R/pull-requests/1");
            expect(h.mockClients.buildStatus.get).toHaveBeenCalledWith("commits/resolved999");
            expect(h.mockClients.buildStatus.get).not.toHaveBeenCalledWith("commits/should-be-ignored");
        });
    });
});
