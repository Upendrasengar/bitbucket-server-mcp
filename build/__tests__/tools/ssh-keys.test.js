...
describe("list_ssh_keys", () => {
    ...
    test("returns keys", async () => {
        ...
        const p = await callAndParse(h.client, "list_ssh_keys", {});
    ...
    test("passes user filter", async () => {
        ...
        expect(h.mockClients.ssh.get).toHaveBeenCalledWith("keys", expect.objectContaining({
...
describe("manage_ssh_keys", () => {
    ...
    test("add key", async () => {
        ...
        const p = await callAndParse(h.client, "manage_ssh_keys", {
            action: "add",
            text: "ssh-rsa AAA...",
        });
        expect(p.id).toBe(1);
    ...
    test("delete key", async () => {
        ...
        const p = await callAndParse(h.client, "manage_ssh_keys", { action: "delete", keyId: 1 });
...
//# sourceMappingURL=ssh-keys.test.js.map