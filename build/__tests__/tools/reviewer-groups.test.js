...
describe("list_reviewer_groups", () => {
    ...
    test("returns reviewer groups", async () => {
        ...
        const parsed = await callAndParse(h.client, "list_reviewer_groups", { project: "P", repository: "r" });
        expect(parsed[0].name).toBe("seniors");
        expect(h.mockClients.api.get).toHaveBeenCalledWith("projects/P/repos/r/settings/reviewer-groups");
    });
...
describe("manage_reviewer_groups", () => {
    ...
    test("creates a reviewer group", async () => {
        ...
        const parsed = await callAndParse(h.client, "manage_reviewer_groups", {
            action: "create",
            project: "P",
            repository: "r",
            name: "team-a",
        ...
        expectCalledWithJson(h.mockClients.api.post, "projects/P/repos/r/settings/reviewer-groups", {
            name: "team-a",
            description: "Team A",
            reviewers: [{ name: "admin" }],
        });
    ...
    test("deletes a reviewer group", async () => {
        ...
        const parsed = await callAndParse(h.client, "manage_reviewer_groups", { action: "delete", project: "P", repository: "r", name: "team-a" });
        expect(parsed.deleted).toBe(true);
        expect(h.mockClients.api.delete).toHaveBeenCalledWith("projects/P/repos/r/settings/reviewer-groups/team-a");
    });
    test.each([
        { action: "delete", mockMethod: "delete", name: "x" },
        { action: "create", mockMethod: "post", name: "x" },
    ...
    test("creates with default project when omitted", async () => {
        ...
        expect(h.mockClients.api.post).toHaveBeenCalledWith("projects/D/repos/r/settings/reviewer-groups", expect.anything());
    });
});
//# sourceMappingURL=reviewer-groups.test.js.map