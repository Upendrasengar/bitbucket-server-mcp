...
describe("list_merge_checks", () => {
    ...
    test("returns merge check hooks with settings", async () => {
        ...
        const parsed = await callAndParse(h.client, "list_merge_checks", { project: "P", repository: "r" });
        expect(parsed).toHaveLength(1);
        expect(parsed[0].key).toBe("com.atlassian.bitbucket.server.bitbucket-build.requiredBuildsMergeCheck");
        expect(parsed[0].name).toBe("Required Builds Merge Check");
        expect(parsed[0].description).toBe("Requires builds to pass");
        expect(h.mockClients.api.get).toHaveBeenCalledWith("projects/P/repos/r/settings/hooks");
        expect(h.mockClients.api.get).toHaveBeenCalledWith("projects/P/repos/r/settings/hooks/com.atlassian.bitbucket.server.bitbucket-build.requiredBuildsMergeCheck/settings");
    });
...
describe("manage_merge_checks", () => {
    ...
    test("configures merge check with settings", async () => {
        ...
        const parsed = await callAndParse(h.client, "manage_merge_checks", {
            project: "P",
            repository: "r",
            hookKey: "com.atlassian.bitbucket.server.bitbucket-build.requiredBuildsMergeCheck",
            settings: { requiredBuildsCount: 2 },
        });
        expect(parsed.requiredBuildsCount).toBe(2);
        expectCalledWithJson(h.mockClients.api.put, "projects/P/repos/r/settings/hooks/com.atlassian.bitbucket.server.bitbucket-build.requiredBuildsMergeCheck/settings", { requiredBuildsCount: 2 });
    });
...
//# sourceMappingURL=merge-checks.test.js.map