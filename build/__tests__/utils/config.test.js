...
describe("parseConfig", () => {
    ...
    describe("baseUrl (equivalence: env / options, boundary: trailing slashes)", () => {
        ...
        test("options.baseUrl overrides BITBUCKET_URL", () => {
            ...
            process.env.BITBUCKET_TOKEN = "t";
            expect(parseConfig({ baseUrl: "https://from-opt.com" }).baseUrl).toBe("https://from-opt.com");
        });
    ...
    describe("startupHealthcheck (BITBUCKET_STARTUP_HEALTHCHECK is strict 'true')", () => {
        ...
        test("options.startupHealthcheck overrides env", () => {
            ...
            expect(parseConfig({ startupHealthcheck: false }).startupHealthcheck).toBe(false);
        });
    });
    describe("cacheTtlMs", () => {
        beforeEach(() => {
    ...
    describe("customHeaders (equivalence: env string format)", () => {
        ...
        test("options.customHeaders overrides env", () => {
            ...
            expect(parseConfig({ customHeaders: { "X-Opt": "o" } }).customHeaders).toEqual({ "X-Opt": "o" });
        });
    ...
    describe("defaultProject", () => {
        ...
        test("options overrides env", () => {
            ...
            expect(parseConfig({ defaultProject: "OPT_PROJ" }).defaultProject).toBe("OPT_PROJ");
        });
    });
});
