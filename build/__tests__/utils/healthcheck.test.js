...
describe("runStartupHealthcheck (via real ky against msw)", () => {
    ...
    const healthcheckWarnings = () => warnSpy.mock.calls
        .map((c) => c[0])
        .filter((s) => s.startsWith("Startup healthcheck:"));
    test("200 response logs a single info line and no healthcheck warnings", async () => {
        server.use(http.get("https://git.example.com/rest/api/1.0/application-properties", () => HttpResponse.json({ version: "8.5.0" })));
            ...
            errors: [
                {
                    ...
                    exceptionName: "com.atlassian.bitbucket.AuthenticationRequiredException",
                },
            ],
        };
        server.use(http.get("https://git.example.com/rest/api/1.0/application-properties", () => HttpResponse.json(body, { status: 401 })));
    ...
    test("403 uses access-denied guidance distinct from 401", async () => {
        server.use(http.get("https://git.example.com/rest/api/1.0/application-properties", () => HttpResponse.json({ errors: [] }, { status: 403 })));
        const clients = createApiClients(baseConfig({ token: "t" }));
        await runStartupHealthcheck(clients);
        const warnings = healthcheckWarnings();
        expect(warnings).toHaveLength(1);
    ...
    test("other HTTP status (500) still emits a healthcheck warning with the status code", async () => {
        server.use(http.get("https://git.example.com/rest/api/1.0/application-properties", () => HttpResponse.text("meltdown", { status: 500 })));
        const clients = createApiClients(baseConfig({ token: "t" }));
        await runStartupHealthcheck(clients);
        const warnings = healthcheckWarnings();
        expect(warnings).toHaveLength(1);
    ...
    test("network-level failure lists the relevant env vars without diagnosing", async () => {
        server.use(http.get("https://git.example.com/rest/api/1.0/application-properties", () => HttpResponse.error()));
        const clients = createApiClients(baseConfig({ token: "t" }));
        await runStartupHealthcheck(clients);
        const warnings = healthcheckWarnings();
        expect(warnings).toHaveLength(1);
    ...
    test("never throws: failures propagate as warnings only", async () => {
        server.use(http.get("https://git.example.com/rest/api/1.0/application-properties", () => HttpResponse.text("fail", { status: 500 })));
        const clients = createApiClients(baseConfig({ token: "t" }));
...
describe("createServer wiring (decision table: startupHealthcheck flag)", () => {
    test.each([
        ...
        let probeCalls = 0;
        server.use(http.get("https://git.example.com/rest/api/1.0/application-properties", () => {
            probeCalls++;
