...
describe("truncateDiff", () => {
    ...
    describe("truncation preserves both file edges", () => {
        ...
        test("hidden lines count equals total minus visible", () => {
            ...
            const shownMatch = result.match(/showing first (\d{1,10}) and last (\d{1,10})/);
        ...
        test("truncated output shows fewer content lines than the original", () => {
            ...
            const contentResultLines = resultLines.filter((l) => l.startsWith("+line"));
    ...
    describe("multi-file diffs", () => {
        test("truncates only files that exceed the limit", () => {
            ...
            const result = truncateDiff(diff, 10);
            for (let i = 0; i < 3; i++)
                expect(result).toContain(`+short${i}`);
    ...
    describe("truncation message content", () => {
        ...
        test("truncation marker is delimited by blank lines", () => {
            ...
            const result = truncateDiff(diff, 10);
            const resultLines = result.split("\n");
            const truncIdx = resultLines.findIndex((l) => l.startsWith("[*** FILE TRUNCATED:"));
            expect(truncIdx).toBeGreaterThan(0);
            expect(resultLines[truncIdx - 1]).toBe("");
            const lastMarkerIdx = resultLines.findIndex((l) => l.startsWith("[*** Use maxLinesPerFile"));
