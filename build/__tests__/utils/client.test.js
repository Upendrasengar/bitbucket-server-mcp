...
describe("createApiClients", () => {
    ...
    describe("Custom headers", () => {
        test("forwards custom headers to every request", async () => {
            const clients = createApiClients(baseConfig({
                token: "t",
    ...
    describe("Accept header (decision table: custom Accept x custom Authorization)", () => {
        ...
        test.each([
            ...
            const clients = createApiClients(baseConfig({ token: "t", customHeaders }));
            await clients.api
                .get("projects")
                .json()
                .catch(() => undefined);
    ...
    describe("URL prefixes (each client targets its REST endpoint)", () => {
        test.each([
            ...
            await clients[clientKey]
                ...
                .json()
                .catch(() => undefined);
            expect(captured[0].url).toBe(`https://git.example.com/${expectedPath}/ping`);
        });
    });
    describe("Retry policy", () => {
        test.each([408, 429, 500, 502, 503, 504])("retries GET on status %i", async (status) => {
            const respond = vi
                .fn()
                .mockReturnValueOnce(new HttpResponse(null, { status }))
                .mockReturnValue(HttpResponse.json({ values: [] }));
            server.use(http.get("https://git.example.com/rest/api/1.0/projects", respond));
            const clients = createApiClients(baseConfig({ token: "t" }));
        ...
        test("does not retry GET on 404", async () => {
            let attempts = 0;
            server.use(http.get("https://git.example.com/rest/api/1.0/projects", () => {
                attempts++;
        ...
        test("does not retry POST on 503", async () => {
            let attempts = 0;
            server.use(http.post("https://git.example.com/rest/api/1.0/projects", () => {
                attempts++;
                return new HttpResponse(null, { status: 503 });
            }));
            const clients = createApiClients(baseConfig({ token: "t" }));
            await expect(clients.api.post("projects", { json: {} }).json()).rejects.toThrow();
            expect(attempts).toBe(1);
        });
    });
    describe("timeout configuration", () => {
        ...
        test("requests time out after the configured timeout", async () => {
            vi.useFakeTimers();
            server.use(http.get("https://git.example.com/rest/api/1.0/projects", async () => {
                await new Promise(() => { });
            }));
            const clients = createApiClients(baseConfig({ token: "t", requestTimeoutMs: 30_000 }));
    ...
    describe("URL redaction (value-based, not key-name heuristic)", () => {
        ...
        test("redacts token value wherever it appears in the logged URL", async () => {
            ...
            const clients = createApiClients(baseConfig({ token: "super-secret-token" }));
            ...
            await clients.api
                ...
                .json()
                .catch(() => undefined);
            expect(debugSpy).not.toHaveBeenCalledWith(expect.stringContaining("super-secret-token"));
            expect(debugSpy).toHaveBeenCalledWith(expect.stringContaining("[REDACTED]"));
        });
        test("redacts password value wherever it appears in the logged URL", async () => {
            const debugSpy = vi.spyOn(logger, "debug");
            const clients = createApiClients(baseConfig({ username: "alice", password: "hunter2" }));
            await clients.api
                .get("projects", { searchParams: { pw: "hunter2" } })
                .json()
                .catch(() => undefined);
            expect(debugSpy).not.toHaveBeenCalledWith(expect.stringContaining("hunter2"));
            expect(debugSpy).toHaveBeenCalledWith(expect.stringContaining("[REDACTED]"));
        });
        test("redacts custom header values wherever they appear in the logged URL", async () => {
            const debugSpy = vi.spyOn(logger, "debug");
            const clients = createApiClients(baseConfig({ customHeaders: { "X-ZTA-Token": "zta-secret-xyz" } }));
            await clients.api
                .get("projects", { searchParams: { tok: "zta-secret-xyz" } })
                .json()
                .catch(() => undefined);
            expect(debugSpy).not.toHaveBeenCalledWith(expect.stringContaining("zta-secret-xyz"));
            expect(debugSpy).toHaveBeenCalledWith(expect.stringContaining("[REDACTED]"));
        });
        test("does not redact innocent values that share a key name with credential params", async () => {
            ...
            expect(debugSpy).toHaveBeenCalledWith(expect.stringContaining("public-value"));
        });
    });
    ...
    describe("retry: only GET requests are retried", () => {
        test("POST on 503 is not retried (only GET)", async () => {
            let attempts = 0;
            server.use(http.post("https://git.example.com/rest/api/1.0/test", () => {
                attempts++;
                return new HttpResponse(null, { status: 503 });
            }));
            const clients = createApiClients(baseConfig({ token: "t" }));
            await expect(clients.api.post("test", { json: {} }).json()).rejects.toThrow();
            expect(attempts).toBe(1);
        });
    });
    ...
    describe("429 X-RateLimit-Reset handling", () => {
        ...
        test("retries 429 without X-RateLimit-Reset without any rate-limit warning", async () => {
            ...
            await clients.api.get("projects").json();
            expect(respond).toHaveBeenCalledTimes(2);
            expect(warnSpy).not.toHaveBeenCalledWith(expect.stringContaining("Rate limited"));
        });
        test("retries 429 with expired X-RateLimit-Reset without any rate-limit warning", async () => {
            ...
            const respond = vi
                .fn()
                .mockReturnValueOnce(new HttpResponse(null, {
                status: 429,
        ...
        test("logs rate-limit warning when X-RateLimit-Reset is in the near future", async () => {
            ...
            await clients.api.get("projects").json();
            expect(respond).toHaveBeenCalledTimes(2);
            expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("Rate limited (429); reset in"));
        });
    });
    describe("getPaginated", () => {
        test("returns first page without second request when isLastPage=true", async () => {
            const respond = vi.fn().mockReturnValue(HttpResponse.json({
                values: [{ id: 1 }],
                isLastPage: true,
                size: 1,
            }));
            server.use(http.get("https://git.example.com/rest/api/1.0/projects/TEST/repos/my-repo/webhooks", respond));
            const clients = createApiClients(baseConfig({ token: "t" }));
            const result = await getPaginated(clients.api, "projects/TEST/repos/my-repo/webhooks");
