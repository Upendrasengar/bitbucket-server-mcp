...
describe("validatePaginated", () => {
    ...
    describe("graceful fallback (warn + return data)", () => {
        test("warns and falls back when isLastPage is missing but values is an array", () => {
            ...
            expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("list_projects"), expect.anything());
            expect(result).toBe(data);
        });
        test("includes context name in the warning message", () => {
            validatePaginated({ values: [] }, "get_branches");
            expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("get_branches"), expect.anything());
        });
    });
    describe("throws on unrecoverable responses", () => {
        test("throws when values is missing entirely", () => {
            expect(() => validatePaginated({ isLastPage: true }, "list_repos")).toThrow("Invalid paginated response from list_repos");
        });
        test("throws when values is not an array", () => {
            expect(() => validatePaginated({ values: "not-an-array", isLastPage: true }, "list_commits")).toThrow("Invalid paginated response from list_commits");
        });
        test("throws for null input", () => {
            expect(() => validatePaginated(null, "list_branches")).toThrow("Invalid paginated response from list_branches");
        });
        test("throws for non-object input", () => {
            expect(() => validatePaginated("string", "test")).toThrow("Invalid paginated response from test");
        });
    });
});
