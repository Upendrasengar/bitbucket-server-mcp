...
import { curateResponse, curateList, DEFAULT_PR_FIELDS, DEFAULT_PROJECT_FIELDS, DEFAULT_REPOSITORY_FIELDS, DEFAULT_BRANCH_FIELDS, DEFAULT_COMMIT_FIELDS, DEFAULT_SEARCH_FIELDS, } from "../../response/curate.js";
describe("curateResponse", () => {
    ...
    describe("top-level field selection", () => {
        ...
        test("skips undefined fields", () => {
            ...
            const result = curateResponse(d, "a,b");
    ...
    describe("nested field paths (object)", () => {
        ...
        test("picks multiple sub-fields at same level", () => {
            expect(curateResponse(data, "author.user.name,author.user.email")).toEqual({ author: { user: { name: "alice", email: "a@x" } } });
        });
    ...
    describe("nested field paths (array)", () => {
        ...
        test("preserves array length (no filtering of items)", () => {
            const result = curateResponse(data, "reviewers.name");
    ...
    describe("null and edge values (boundary)", () => {
        ...
        test("nested null is skipped (treated as not-an-object)", () => {
            const d = { author: null };
            expect(curateResponse(d, "author.name")).toEqual({});
        });
