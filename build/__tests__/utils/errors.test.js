...
import ky from "ky";
import { formatApiError, extractBitbucketMessage, handleToolError, } from "../../http/errors.js";
...
describe("formatApiError (status code)", () => {
    ...
    test.each([500, 502, 503, 504, 599])("5xx status %i has server-error guidance distinct from 4xx", (status) => {
        ...
        expect(serverResult).toMatch(/Bitbucket server error|temporarily unavailable/i);
    });
    test.each([418, 451, 200])("unmapped status %i includes the status code in the message", (status) => {
        const result = formatApiError(status, "msg");
...
describe("extractBitbucketMessage (equivalence classes over response bodies)", () => {
            ...
            errors: [
                {
                    ...
                    exceptionName: "com.atlassian.bitbucket.pull.PullRequestAlreadyMergedException",
                },
            ],
        };
        const result = extractBitbucketMessage(body);
        expect(result).toContain("com.atlassian.bitbucket.pull.PullRequestAlreadyMergedException");
    ...
    test("fallback to generic .message when no errors array", () => {
        expect(extractBitbucketMessage({ message: "plain message" })).toBe("plain message");
    });
...
describe("handleToolError (real ky HTTPError via msw)", () => {
    ...
    async function throwHttpError(path) {
        try {
    ...
    test("Bitbucket error body: exceptionName + message end up in output", async () => {
        ...
        server.use(http.get("https://git.example.com/rest/api/1.0/pulls", () => HttpResponse.json(body, { status: 404 })));
        const error = await throwHttpError("pulls");
        const result = handleToolError(error);
        expect(result.isError).toBe(true);
        expect(result.content[0].text).toContain("com.atlassian.bitbucket.pull.NoSuchPullRequestException");
    ...
    test("structured Bitbucket error body is returned directly without generic guidance", async () => {
        const body = {
            errors: [
                {
                    message: "The pull request has already been updated since the version you sent.",
                    exceptionName: "com.atlassian.bitbucket.pull.PullRequestOutOfDateException",
                },
            ],
        };
        server.use(http.get("https://git.example.com/rest/api/1.0/pulls", () => HttpResponse.json(body, { status: 409 })));
        const error = await throwHttpError("pulls");
        const result = handleToolError(error);
    ...
    test("500 body keeps server-error guidance when body has no errors array", async () => {
        server.use(http.get("https://git.example.com/rest/api/1.0/foo", () => HttpResponse.text("internal meltdown", { status: 500 })));
    ...
    test("empty body falls back to ky's own message, not the literal empty string", async () => {
        server.use(http.get("https://git.example.com/rest/api/1.0/foo", () => new HttpResponse(null, { status: 500 })));
        const error = await throwHttpError("foo");
        const result = handleToolError(error);
        expect(result.isError).toBe(true);
    ...
    ])("$name: falls through to formatApiError with status guidance", async ({ body, expectGuidance }) => {
        server.use(http.get("https://git.example.com/rest/api/1.0/pulls", () => HttpResponse.json(body, { status: 404 })));
        const error = await throwHttpError("pulls");
        const result = handleToolError(error);
        expect(result.isError).toBe(true);
    ...
    test.each([
        ...
        ["null", null],
        ["undefined", undefined],
    ])("non-HTTPError primitive (%s) is stringified into the output", (_name, value) => {
    ...
    test("always returns isError: true", async () => {
        ...
        server.use(http.get("https://git.example.com/rest/api/1.0/foo", () => HttpResponse.json(body, { status: 400 })));
