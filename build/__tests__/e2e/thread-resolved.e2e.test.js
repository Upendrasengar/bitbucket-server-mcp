...
import { VERSIONS_WITH_THREAD_RESOLVED, VERSIONS_WITHOUT_THREAD_RESOLVED, } from "./versions.js";
import { startBitbucket, } from "./bitbucket-container.js";
import { bootstrap, createComment } from "./bootstrap.js";
 ...
 */
describe.each(VERSIONS_WITH_THREAD_RESOLVED)("threadResolved supported: Bitbucket $name", (version) => {
    let bb;
    let mcp;
    let scenario;
    beforeAll(async () => {
    ...
    test("manage_comment edit {threadResolved:true} flips the flag without touching state/severity", async () => {
        ...
        const updated = await callAndParse(mcp.client, "manage_comment", {
            action: "edit",
    ...
    test("manage_comment edit {state:RESOLVED, threadResolved:true} updates both in one call", async () => {
        ...
        const blocker = await callAndParse(mcp.client, "manage_comment", {
            action: "edit",
            project: scenario.projectKey,
            repository: scenario.repoSlug,
            prId: scenario.prId,
        ...
        const resolved = await callAndParse(mcp.client, "manage_comment", {
            action: "edit",
            project: scenario.projectKey,
            repository: scenario.repoSlug,
            prId: scenario.prId,
...
describe.each(VERSIONS_WITHOUT_THREAD_RESOLVED)("threadResolved unsupported: Bitbucket $name", (version) => {
    let bb;
    let mcp;
    let scenario;
    beforeAll(async () => {
...
//# sourceMappingURL=thread-resolved.e2e.test.js.map