...
describe.each(SELECTED_VERSIONS)("refs: Bitbucket $name", (version) => {
    ...
    test("list_branches returns main and feature", async () => {
        const parsed = await callAndParse(mcp.client, "list_branches", {
    ...
    test("manage_branches create creates a new branch", async () => {
        const parsed = await callAndParse(mcp.client, "manage_branches", {
            action: "create",
            project: s.projectKey,
            repository: s.repoSlug,
            branch: "e2e-branch",
    ...
    test("get_commit returns the main commit", async () => {
        const parsed = await callAndParse(mcp.client, "get_commit", {
            project: s.projectKey,
            repository: s.repoSlug,
    ...
    test("list_tags returns paginated result", async () => {
        const parsed = await callAndParse(mcp.client, "list_tags", {
            project: s.projectKey,
            repository: s.repoSlug,
        });
    ...
    test("manage_tags create creates a tag", async () => {
        const parsed = await callAndParse(mcp.client, "manage_tags", {
            action: "create",
            project: s.projectKey,
            repository: s.repoSlug,
            name: "e2e-tag",
...
//# sourceMappingURL=refs.e2e.test.js.map