...
export const VERSIONS = [
    ...
    { name: "9.4", image: "atlassian/bitbucket:9.4" },
    {
        ...
        extraEnv: {
            JVM_SUPPORT_RECOMMENDED_ARGS: "-Dcom.atlassian.plugins.authentication.basic.auth.filter.force.allow=true",
        },
    },
];
/**
...
export function gte(v, min) {
    return compareVersions(v.name, min) >= 0;
}
export function lt(v, min) {
    return compareVersions(v.name, min) < 0;
}
/**
...
export const VERSIONS_WITH_THREAD_RESOLVED = SELECTED_VERSIONS.filter((v) => gte(v, THREAD_RESOLVED_SINCE));
export const VERSIONS_WITHOUT_THREAD_RESOLVED = SELECTED_VERSIONS.filter((v) => lt(v, THREAD_RESOLVED_SINCE));
/** Labels were introduced in Bitbucket Server 5.13, below our minimum of 7.21. */
export const LABELS_SINCE = "7.21";
export const VERSIONS_WITH_LABELS = SELECTED_VERSIONS.filter((v) => gte(v, LABELS_SINCE));
export const VERSIONS_WITHOUT_LABELS = SELECTED_VERSIONS.filter((v) => lt(v, LABELS_SINCE));
