...
describe.each(SELECTED_VERSIONS)("pull requests: Bitbucket $name", (version) => {
    let bb;
    let mcp;
    let s;
    beforeAll(async () => {
    ...
    test("get_pull_request_commits returns data", async () => {
        const r = await callAndParse(mcp.client, "get_pull_request_commits", {
    ...
    test("list_pull_requests returns PR properties in the curated output", async () => {
        const r = await callAndParse(mcp.client, "list_pull_requests", {
            project: s.projectKey,
            repository: s.repoSlug,
        });
    ...
    test("create_pull_request with draft:true", async () => {
        ...
        await bb.api.put(`projects/${s.projectKey}/repos/${s.repoSlug}/browse/draft.md`, { body: form });
        const r = await callAndParse(mcp.client, "create_pull_request", {
            project: s.projectKey,
            repository: s.repoSlug,
        ...
        const pr = await bb.api
            ...
            .json();
        await bb.api
            .post(`projects/${s.projectKey}/repos/${s.repoSlug}/pull-requests/${r.id}/decline`, { json: { version: pr.version } })
            .catch(() => { });
    });
});
//# sourceMappingURL=pull-requests.e2e.test.js.map