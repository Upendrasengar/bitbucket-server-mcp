...
const SHARED_DIR_IN_CONTAINER = "/var/atlassian/application-data/bitbucket/shared";
/**
 ...
 */
function renderProperties(v) {
    const license = process.env.BITBUCKET_TIMEBOMB_LICENSE;
    if (!license) {
        throw new Error("BITBUCKET_TIMEBOMB_LICENSE env var is required. Copy the 10-user Bitbucket Data Center timebomb license from https://developer.atlassian.com/platform/marketplace/timebomb-licenses-for-testing-server-apps/ and export it.");
    }
    const props = {
 ...
 */
async function waitForAuthenticatedApi(baseUrl, timeoutMs) {
    const api = ky.create({
    ...
    throw new Error(`Bitbucket did not reach authenticated-ready within ${timeoutMs}ms. Last error: ${String(lastError)}`);
}
/**
 ...
 */
export async function startBitbucket(version) {
    ...
    await writeFile(join(hostSharedDir, "bitbucket.properties"), renderProperties(version), { mode: 0o644 });
    ...
    return {
        ...
        async stop() {
            ...
            await container.exec(["chown", "-R", `${uid}:${gid}`, SHARED_DIR_IN_CONTAINER], { user: "root" });
            await container.stop();
            await rm(hostSharedDir, { recursive: true, force: true });
        },
    };
}
//# sourceMappingURL=bitbucket-container.js.map