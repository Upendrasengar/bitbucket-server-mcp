...
export function parseConfig(options) {
    ...
    if (!token && !(username && password)) {
        throw new Error("Authentication is required. Provide BITBUCKET_TOKEN or both BITBUCKET_USERNAME and BITBUCKET_PASSWORD.");
    }
    ...
    return {
        ...
        maxLinesPerFile: options?.maxLinesPerFile ??
            (maxLinesRaw ? parseInt(maxLinesRaw, 10) : undefined),
        readOnly: options?.readOnly ?? readOnlyRaw === "true",
        customHeaders: options?.customHeaders ??
            parseCustomHeaders(process.env.BITBUCKET_CUSTOM_HEADERS),
        enabledTools: options?.enabledTools ??
            (enabledToolsRaw
                ? enabledToolsRaw.split(",").map((t) => t.trim())
                : undefined),
        cacheTtlMs: options?.cacheTtlMs ??
            (cacheTtlRaw ? parseInt(cacheTtlRaw, 10) * 1000 : 5 * 60 * 1000),
        requestTimeoutMs: options?.requestTimeoutMs ??
            (requestTimeoutRaw ? parseInt(requestTimeoutRaw, 10) : 30_000),
        startupHealthcheck: options?.startupHealthcheck ?? healthcheckRaw === "true",
    };
}
