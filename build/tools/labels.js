...
const labelActions = {
    ...
    remove: async ({ clients, resolvedProject, repository, name }) => {
        await clients.api.delete(`projects/${resolvedProject}/repos/${repository}/labels/${name}`);
        return formatResponse({ deleted: true, label: name });
    },
};
export function registerLabelTools(ctx) {
    ...
    server.registerTool("list_labels", {
        ...
        try {
            ...
            const data = await getPaginated(clients.api, `projects/${resolvedProject}/repos/${repository}/labels`, { searchParams: { limit, start } });
    ...
    server.registerTool("manage_labels", {
        description: 'Manage repository labels. Actions: "add" (create a new label), "remove" (delete a label).',
        inputSchema: {
