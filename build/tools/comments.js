...
const commentActions = {
    create: async ({ clients, basePath, text, parentId, state, severity, filePath, line, lineType, diffType, fileType, }) => {
        const body = {
            text,
    ...
    edit: async ({ clients, basePath, commentId, text, version, severity, state, threadResolved, }) => {
        const body = {
            text,
            version,
            ...(severity && { severity }),
    ...
    react: async ({ clients, resolvedProject, repository, prId, commentId, emoticon, }) => {
...
export function registerCommentTools(ctx) {
    ...
    server.registerTool("manage_comment", {
        ...
        inputSchema: {
            ...
            version: z
                .number()
                .optional()
                .describe("Comment version for optimistic locking (required for edit and delete)."),
            parentId: z
                .number()
                .optional()
            ...
            state: z
                ...
                .optional()
                .describe("Comment state. PENDING = draft (create only). RESOLVED = mark as resolved (edit only). OPEN = reopen (edit only)."),
            severity: z
                .enum(["NORMAL", "BLOCKER"])
                .optional()
                .describe("Comment severity. BLOCKER marks it as a task (create and edit)."),
            threadResolved: z
                .boolean()
                .optional()
                .describe("Close or reopen the comment thread (edit only). Independent of `state`. Requires Bitbucket Data Center >= 8.9; older servers accept the PUT but ignore the field."),
            filePath: z
                .string()
                .optional()
            ...
            lineType: z
                ...
                .optional()
                .describe("Type of line being commented on. ADDED = new line, REMOVED = deleted line, CONTEXT = unchanged line visible in the diff."),
            diffType: z
                .enum(["EFFECTIVE", "RANGE", "COMMIT"])
                .optional()
                .describe("Which diff to anchor the comment on. EFFECTIVE = overall PR diff (default). COMMIT = a single commit's diff. RANGE = diff between two specific commits."),
            fileType: z
                .enum(["TO", "FROM"])
                .optional()
                .describe("Which side of the diff. TO = new version (default). FROM = old version (useful for renames)."),
            emoticon: z
                .string()
                .optional()
                .describe("Emoticon shortcut for react/unreact (e.g. thumbsup, heart, tada). Use search_emoticons to find available options."),
        },
    ...
    server.registerTool("search_emoticons", {
        description: "Search available emoticons for comment reactions. Returns matching shortcut names to use with manage_comment react/unreact.",
        inputSchema: {
